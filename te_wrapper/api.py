...
class ThousandEyesAPI:
    ...
    def get_test_results_by_link(self, results_href: str) -> TestResult:
        """
        Fetches the latest test result for a given results link.

        Args:
            results_href: The href link to the test results.

        Returns:
            A TestResult object containing the latest result data.

        Raises:
            RuntimeError: If no results are returned for the link.
        """
        data = self._request("GET", results_href.lstrip("/"), params={"limit": 1})

        results_list = data.get("results") or data.get("testResults") or []
        if not results_list:
            raise RuntimeError(f"No results returned for link {results_href}")
